...
const drawBackground = (
    ...
    const radius = getCircleRadius(
        canvasWidth,
        canvasHeight,
        CANVAS_CIRCLE_MARGIN
    );
    const bigCircle = new Circle(
        bigCircleCenter,
        radius,
        DEFAULT_LINE_WIDTH,
        DEFAULT_DRAWINGS_COLOR
    );
    bigCircle.drawOnCanvas(ctx);

    const handlerCircleCenter = {
        x: bigCircleCenter.x + radius,
        y: bigCircleCenter.y,
    };
    ...
    const guideArrowEdge = {
        x:
            bigCircleCenter.x +
            curveArrowDistance * Math.cos(curveArrowEndAngle),
        y:
            bigCircleCenter.y +
            curveArrowDistance * Math.sin(curveArrowEndAngle),
    };
...
const highlightRightAngle = (
    ...
    const rightAngleMarkerX =
        bigCircleCenter.x + radius * ANGLE_MARKER_WIDTH_FACTOR;
    const rightAngleMarkerY =
        handlerCircleCenter.y < bigCircleCenter.y
...
export function PlayGroundCanvas() {
    ...
    const [canvasDimension, setCanvasDimension] = useState<Dimension | null>(
        null
    );
    ...
    const handleNativeMouseMoveRef = useRef<(e: MouseEvent) => void>(
        null
    ) as MutableRefObject<(e: MouseEvent) => void>;
    const handleNativeTouchMoveRef = useRef<(e: TouchEvent) => void>(
        null
    ) as MutableRefObject<(e: TouchEvent) => void>;
    ...
    const calculateCanvasDimension = (): Dimension => {
        ...
        const canvasHeight =
            shorterParentSide -
            HEADER_APPROX_MARGIN -
            ANGLE_INDICATOR_APPROX_MARGIN;
    ...
    const drawInitialState = (
        ...
        const handlerCircleCenter = drawBackground(
            canvasContext,
            canvasDimension,
            false
        );
        // Draw the handler circle
        const handlerCircle = new Circle(
            handlerCircleCenter,
            HANDLER_CIRCLE_RADIUS,
            DEFAULT_LINE_WIDTH,
            DEFAULT_DRAWINGS_COLOR,
            DEFAULT_DRAWINGS_COLOR,
            true
    ...
    const drawAngleChange = useCallback(
        (mouseOrTouchPoint: Point) => {
            const xyCoordsInCanvas = getContainerCoords(
                mouseOrTouchPoint,
            ...
            const adjustedWithCenter = {
                x: xyCoordsInCanvas.x - canvasCenter.x,
                y: xyCoordsInCanvas.y - canvasCenter.y,
            };
            const distanceFromCenter = Math.sqrt(
                Math.pow(adjustedWithCenter.x, 2) +
                    Math.pow(adjustedWithCenter.y, 2)
            );
            const cosineValue = adjustedWithCenter.x / distanceFromCenter;
            const sineValue = adjustedWithCenter.y / distanceFromCenter;
            const pointOnArc = {
                x: radius * cosineValue,
                y: radius * sineValue,
            } as Point;

            // Convert to the actual canvas coordinates
            const pointOnArcInXY = {
                x: pointOnArc.x + canvasCenter.x,
                y: pointOnArc.y + canvasCenter.y,
            } as Point;
            ...
            const angleInRadian = getAngleInRadian(
                cosineValue,
                sineValue,
                true
            );
            ...
            if (angleInDegreeForUI == 180) {
                ...
                ctx.arc(
                    canvasCenter.x,
                    canvasCenter.y,
                    radius * ANGLE_MARKER_WIDTH_FACTOR,
                    0,
                    angleInRadian,
                    true
                );
            ...
            ctx.arc(
                ...
                radius,
                0,
                getAngleInRadian(cosineValue, sineValue, true),
                true
            );
            ctx.closePath();
            ctx.fill();
    ...
    const startDragging = useCallback(
        (mouseOrTouchPoint: Point, addEventListener: () => void) => {
            if (!initialized || !handlerCircle) {
                console.log('Not properly initialized');
                return;
            }

            const coordsInCanvas = getContainerCoords(
                mouseOrTouchPoint,
                canvasRef.current!.getBoundingClientRect()
            );
        ...
        (e: React.MouseEvent<HTMLCanvasElement>) => {
            startDragging({ x: e.clientX, y: e.clientY }, () => {
                ...
                canvasRef.current?.addEventListener(
                    'mousemove',
                    handleNativeMouseMoveRef.current
                );
            });
        },
    ...
    const handleTouchStart = useCallback(
        (e: React.TouchEvent<HTMLCanvasElement>) => {
            const touchList = e.changedTouches[0];
            startDragging(
                { x: touchList.clientX, y: touchList.clientY },
                () => {
                    handleNativeTouchMoveRef.current = handleNativeTouchMove;
                    canvasRef.current?.addEventListener(
                        'touchmove',
                        handleNativeTouchMoveRef.current
                    );
                }
            );
        },
    ...
    useEffect(() => {
        if (canvasContext != null) {
            const handlerCircle = drawInitialState(
                canvasContext,
                canvasDimension ?? {
